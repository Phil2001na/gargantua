...
Flight: **W/S** main drive, **A/D** yaw, drag or arrow keys to steer, **Q/E** roll, **Shift** boost, **X** brake. The ship keeps its momentum; a flight-assist trims sideways drift, and speed is automatically limited near worlds and inside the bridge. **C** cycles chase, hull and cockpit cameras; scroll zooms the chase camera. **Orbit** circles the selected body. On mobile use Thrust/Reverse/Brake and drag to steer. **Space / Pause** stops animation and programmed travel; manual flight remains available. **H** hides the interface; Escape or Show controls restores it. **F** toggles fullscreen. Sound and image export are in the atlas toolbar. Tab opens the route chart from the flight canvas; native modal keyboard navigation works within the chart.
